...
(defun %validate-http-request-url-argument (url)
  (unless (and (stringp url) (plusp (length url)))
    (error 'ollama-argument-error
           :message "HTTP request URL must be a non-empty string."))
...
(defun %validate-http-status (status)
  (unless (typep status '(integer 100 599))
    (error 'ollama-argument-error
           :message "HTTP response status must be an integer from 100 through 599."))
  status)

(defun %validate-http-response-body (body stream)
  (when (and body stream)
    (error 'ollama-argument-error
           :message "An HTTP response cannot contain both a body and a stream."))
  (when (and body
             (not (or (stringp body)
                      (typep body '(vector (unsigned-byte 8))))))
    (error 'ollama-argument-error
           :message "HTTP response body must be a string or octet vector."
           :detail (type-of body)))
  body)
...
(defun %validate-http-response-stream (stream)
  (when (and stream (not (streamp stream)))
    (error 'ollama-argument-error
           :message "HTTP response streams must be stream objects."))
  stream)

(defun %validate-close-function (close-function)
  (when (and close-function (not (functionp close-function)))
    (error 'ollama-argument-error
           :message "HTTP response close callbacks must be functions."))
  close-function)
...
(defun make-http-request (&rest options)
  ...
  (%validate-keyword-options options
                             '(:method :url :headers :body :stream-p))
  (let ((method (%keyword-option options :method :get))
    ...
    (%make-http-request :method (%normalize-method method)
                        :url url
                        :headers (%normalize-headers headers)
                        :body body
                        :stream-p (not (null stream-p)))))
...
(defun %close-error (condition)
  (if (typep condition 'ollama-transport-error)
      ...
      (make-condition 'ollama-transport-error
                      :message "Failed to close an HTTP response resource."
                      :cause condition)))
...
(defun %close-owned-resources (stream close-function)
  (let ((failure nil))
    (when (streamp stream)
      (setf failure
            (%close-resource (lambda () (close stream)) failure)))
    (when close-function
      (setf failure
            (%close-resource close-function failure)))
    (when failure
      (error failure))))
