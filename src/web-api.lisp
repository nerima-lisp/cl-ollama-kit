(defun %validate-web-query (query)
  (unless (and (stringp query) (plusp (length query)))
    (error 'ollama-argument-error
           :message
           "Web search queries must be non-empty strings."))
  query)

(defun %validate-web-max-results (max-results)
  (unless (and (integerp max-results) (<= 1 max-results 10))
    (error 'ollama-argument-error
           :message
           "Web search max-results must be an integer from 1 through 10."))
  max-results)
...
(defun %validate-web-url (url)
  (unless (and (stringp url) (plusp (length url)))
    (error 'ollama-argument-error
           :message
           "Web fetch URLs must be non-empty strings."))
  url)

(defun web-search (client query
                          &key
                          (max-results +json-unspecified+)
...
`/api` base URL and an API key when using the hosted service."
  ...
  (%request-json-value client
                       ...
                       "/web_search"
                       :body
                       (if (eq max-results +json-unspecified+)
...
(defun web-fetch (client url &key (timeout +timeout-unspecified+) headers)
