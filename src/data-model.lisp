...
(defstruct (client
             (:constructor %make-client
                 (&key base-url network-boundary headers timeout max-input-length
                       max-request-length api-key-p allow-insecure-http)))
...
(defparameter +json-unspecified+ (gensym "JSON-UNSPECIFIED-"))

(defparameter +timeout-unspecified+ (gensym "TIMEOUT-UNSPECIFIED-"))

(defstruct (http-request
             (:constructor %make-http-request
                 (&key method url headers body stream-p)))
...
(defstruct (ollama-stream
            (:constructor %make-ollama-stream
                (&key stream response close-function wire-format
                      max-line-length)))
  stream
  response
  close-function
...
(defparameter +max-sse-event-lines+ 65536
