...
(defun %normalize-method (method)
  (let ((name (typecase method
                ((or keyword symbol) (symbol-name method))
                (string method)
                (t (error 'ollama-argument-error
                          :message "HTTP methods must be strings or symbols.")))))
    (unless (and (plusp (length name))
                 (every #'%http-token-character-p name))
      (error 'ollama-argument-error
             :message "HTTP methods must be non-empty valid tokens."))
    (let* ((normalized-name (string-upcase name))
           (known-method (assoc normalized-name +standard-http-methods+
                                :test #'string=)))
...
(defun %normalize-header-name (name)
  (let ((normalized
          (typecase name
            ...
            (t (error 'ollama-argument-error
                      :message "HTTP header names must be strings or symbols.")))))
    (unless (and (plusp (length normalized))
                 (every #'%http-token-character-p
                       normalized))
      (error 'ollama-argument-error
             :message "HTTP header names must be valid field-name tokens."))
    normalized))

(defun %normalize-header-entry (entry)
  (unless (consp entry)
    (error 'ollama-argument-error
           :message "Each header must be a cons of name and value."))
  (let ((name (%normalize-header-name (car entry)))
        (value (cdr entry)))
    (unless (stringp value)
      (error 'ollama-argument-error
             :message "HTTP header values must be strings."))
    (when (%unsafe-header-value-p value)
      (error 'ollama-argument-error
             :message "HTTP header values must not contain controls."))
    (cons name value)))

(defun %normalize-headers (headers)
  (unless (listp headers)
    (error 'ollama-argument-error
           :message "Headers must be an alist of name . value pairs."))
...
(defun %ensure-header (headers name value)
  (unless (and (stringp value)
               (not (%unsafe-header-value-p value)))
    (error 'ollama-argument-error
           :message "HTTP header values must be control-free strings."))
...
(defun %validate-boolean (value name)
  (unless (or (null value) (eq value t))
    (error 'ollama-argument-error
           :message (format nil "~A must be NIL or T." name)
           :detail value))
  value)
...
(defun %validate-timeout (value)
  (unless (or (null value)
              (and (realp value)
                   ...
                   (not (minusp value))))
    (error 'ollama-argument-error
           :message "TIMEOUT must be NIL or a non-negative real number."
           :detail value))
  value)
