(defun %validate-api-key (api-key)
  ...
  (when (and api-key
             (or (zerop (length api-key))
                 (%unsafe-header-value-p api-key)))
    (error 'ollama-argument-error
           :message "API-KEY must be a non-empty header-safe string."))
  api-key)

(defun %validate-client-limits (max-input-length max-request-length)
  (unless (typep max-input-length '(integer 1 *))
    (error 'ollama-argument-error
           :message "MAX-INPUT-LENGTH must be a positive integer."))
  (unless (typep max-request-length '(integer 1 *))
    (error 'ollama-argument-error
           :message "MAX-REQUEST-LENGTH must be a positive integer."))
  (values max-input-length max-request-length))

(defun %validate-client-options (network-boundary api-key timeout
                                 max-input-length max-request-length
                                 allow-insecure-http)
  (unless network-boundary
    (error 'ollama-argument-error
           :message "A NETWORK-BOUNDARY is required."))
...
(defun %client-headers-with-auth (normalized-headers api-key)
  (when (and api-key (%header-present-p "Authorization" normalized-headers))
    (error 'ollama-argument-error
           :message "Do not combine API-KEY with an Authorization header."))
  (if api-key
...
choose and configure their concrete HTTP implementation explicitly." 
  ...
  (multiple-value-bind (base-url network-boundary headers api-key timeout
                        max-input-length max-request-length allow-insecure-http)
      (%client-option-values options)
    (%validate-client-options network-boundary api-key timeout
                              max-input-length max-request-length
                              allow-insecure-http)
