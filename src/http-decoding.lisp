      ...
      (multiple-value-bind (text remainder)
        ...
        (if (zerop (length remainder))
            ...
            (error 'ollama-protocol-error
                   :message "HTTP response body ends with an incomplete UTF-8 sequence."
                   :detail remainder)))
    (ollama-protocol-error (condition)
      (error condition))
    (cl-codec-kit:cl-codec-kit-error (condition)
      (error 'ollama-protocol-error
             :message "HTTP response body is not valid UTF-8."
             :detail condition))))

(defun %validate-input-length (max-input-length)
  (when (and max-input-length
             (not (typep max-input-length '(integer 1 *))))
...
(defun %response-body-text (body)
  (cond
    ...
    ((%octet-vector-p body)
     (%decode-utf8 body))
    (t (error 'ollama-protocol-error
              :message "HTTP response body is neither text nor octets."
              :detail body))))
...
(defun %binary-stream-p (stream)
  (multiple-value-bind (subtype-p known-p)
      (subtypep (stream-element-type stream)
                '(unsigned-byte 8))
  ...
  (with-output-to-string (output)
    (let ((length 0))
      (loop for character = (read-char stream nil nil)
            while character
            do (incf length (%utf8-character-octets character))
  ...
  (or (http-response-stream response)
      (let ((body (http-response-body response)))
        (cond
          ...
          (t (error 'ollama-protocol-error
                    :message "Streaming response body has an unsupported type."
                    :detail body))))))

(defun %http-error-data (body max-input-length)
  (handler-case
      (%parse-json-text body max-input-length)
    (ollama-protocol-error () nil)))
  ...
  (let ((message (%http-error-message data)))
    (if message
        ...
        (error 'ollama-http-error
               :message (format nil "Ollama request failed with HTTP status ~A"
                                status)
               :status status
               :response response
               :body body))))
