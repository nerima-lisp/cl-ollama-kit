                ...
                (let ((client
                       (client-with-request-function
                        (lambda (request &key timeout)
                          ...
                          (response-with-text "{}")))))
                  (let ((response
                         (perform-request client :get "/version" :timeout 17)))
              ...
              (assert
               (%signals-p 'ollama-argument-error
                           (lambda ()
                             (make-http-request "unsupported" t)))))
          (it
           "supports bodyless raw requests and closes malformed JSON responses"
           (let ((client
                  (client-with-request-function
                   (lambda (request &key timeout)
                     (declare (ignore request timeout))
                       ...
                       (lambda (request &key timeout)
                         ...
                         (make-http-response :status
                                             ...
                                             (lambda ()
                                               (incf closed)))))))
                (with-json-response
                 (value response (request-json client :get "/version"))
