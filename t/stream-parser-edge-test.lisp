  ...
  (it "handles character and binary line EOF and limits"
    ...
    (%with-temporary-binary-input
     ...
     (lambda (stream)
       (assert (%signals-p 'ollama-protocol-error
                           (lambda ()
                             (ollama-kit::%read-octet-line-limited
                              stream
                              3)))))))
  ...
  (it "validates stream collection bounds and close idempotence"
    ...
    (let ((stream
            (open-ollama-stream
             ...
             "/generate")))
      (assert (equal t
                     (gethash "ok" (car (stream-events stream)))))
      (assert (stream-closed-p stream)))
    (assert (%signals-p 'ollama-argument-error
                        (lambda () (stream-next nil))))
    (assert (%signals-p 'ollama-argument-error
                        (lambda () (stream-close nil))))
    (let* ((closed 0)
          (client
            (queued-client
             (make-http-response
              :status 200
              :body (cl-codec-kit:string-to-octets
                     (format nil "{\"ok\":true}~%")
                     :encoding :utf-8)
            ...
            (open-ollama-stream
             (queued-client
              (make-http-response
               ...
               :body (cl-codec-kit:string-to-octets
                      "{" :encoding :utf-8)
               :close-function (lambda () (error "stream close failure"))))
             :post
             "/generate"))
          (warning-seen nil))
      (handler-bind
