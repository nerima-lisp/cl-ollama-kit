...
(describe "client and transport boundary"
  (it "serializes UTF-8 JSON through an injected boundary"
    (let ((captured nil)
      ...
      (multiple-value-bind (value http-response)
          (request-json client :post "/chat"
                        :body (json-object "model" "tiny" "prompt" "日本😀"))
        ...
        (assert (equal "http://localhost:11434/api/chat"
                       (http-request-url captured)))
        (let ((payload (json-kit:parse
      ...
      (let ((client
              (client-with-request-function
               (lambda (request &key timeout)
                 ...
                 (response-with-text "{}"))
               :api-key "secret")))
        (request-json client :post "/generate" :body (json-object "x" 1)))
      (assert (equal "Bearer secret"
                     (cdr (assoc "Authorization" (http-request-headers captured)
                                 :test #'string-equal))))
      (assert (equal "application/json; charset=utf-8"
                     (cdr (assoc "Content-Type" (http-request-headers captured)
                                 :test #'string-equal))))
      (assert (equal "application/json"
                     (cdr (assoc "Accept" (http-request-headers captured)
                                 :test #'string-equal))))))

  (it "keeps raw request bodies and transfers response ownership"
    (let* ((body (make-array 3
  ...
  (it "checks and uploads blobs through the raw API boundary"
    (let ((captured nil))
      (let ((client
        ...
        (assert (equal "http://localhost:11434/api/blobs/sha256:present"
                       (http-request-url captured))))
    (assert (not
             (blob-exists-p
  ...
  (it "signals structured API errors and preserves the raw body"
    (let ((condition nil))
      (handler-case
          (request-json (queued-client (response-with-text
                                        "{\"error\":\"model missing\"}"
                                        :status 404))
                        :get "/tags")
      ...
      (assert (equal "model missing" (ollama-error-message condition)))
  ...
  (it "treats redirects and plain error bodies as failures"
    (dolist (fixture (list (cons 302 "redirect") (cons 500 "server error")))
      (let ((condition nil))
        (handler-case
            (request-json (queued-client
                           (response-with-text (cdr fixture)
                                                :status (car fixture)))
                          :get "/version")
          (ollama-http-error (caught) (setf condition caught)))
        (assert condition)
        (assert (= (car fixture) (ollama-http-error-status condition)))
        (assert (equal (cdr fixture) (ollama-http-error-body condition))))))

  (it "rejects a client without a transport boundary"
    (let ((signalled nil))
      (handler-case (make-client)
        (ollama-argument-error () (setf signalled t)))
      (assert signalled)))

  (it "rejects unsafe HTTP header names and values"
    (dolist (headers (list (list (cons "Bad Header" "value"))
                           (list (cons "X-Test"
                                        (format nil "ok~C~CInjected: yes"
                                                #\Return #\Linefeed)))))
      (let ((signalled nil))
        (handler-case
            (client-with-request-function
  ...
  (it "normalizes endpoint slashes without changing the base URL"
    (let ((captured nil))
      ...
      (assert (equal "http://127.0.0.1:11434/api/version"
                     (http-request-url captured)))))

  (it "rejects unsafe endpoints and ambiguous authorization"
    (dolist (base-url (list "http:///api"
                            "http://[::1/api"
                            "http://:11434/api"
          ...
          (client
            (client-with-request-function
             (lambda (request &key timeout)
               ...
               (setf called request)
               (response-with-text "{}"))
             :max-request-length 4)))
      (handler-case
          (request-json client :post "/chat"
                        :body (json-object "x" "123"))
        (ollama-argument-error () nil))
      (handler-case
          (perform-request client :post "/chat"
      ...
      (handler-case
          (request-json
           (client-with-request-function
            ...
            :max-input-length 4)
           :get "/version")
        (ollama-protocol-error (caught) (setf condition caught)))
      (assert condition)))

  (it "reports malformed JSON and invalid UTF-8 as protocol errors"
    (let ((invalid-utf8 (make-array 1
                                    :element-type '(unsigned-byte 8)
                                    :initial-element 255)))
      (dolist (response (list (response-with-text "{broken")
                              (make-http-response
                               :status 200
                               :body invalid-utf8)))
        (let ((condition nil))
          (handler-case
              (request-json (queued-client response) :get "/version")
            (ollama-protocol-error (caught) (setf condition caught)))
          (assert condition))))))
