...
(describe "NDJSON response streams"
  ...
  (it "signals malformed NDJSON instead of returning a partial event"
    (let ((stream
          ...
          (signalled nil))
      (handler-case (stream-next stream)
        (ollama-protocol-error () (setf signalled t)))
      (assert signalled)))
              ...
              (lambda (request &key timeout)
                ...
                (make-http-response
                 ...
                 (make-string-input-stream
                  (format nil "data: {\"aaaaa\":~%data: 123456}~%~%"))))
              :max-input-length 16)
             :post "/chat/completions"
             :body (json-object "stream" t)))
          (signalled nil))
      (handler-case (stream-next stream)
        (ollama-protocol-error () (setf signalled t)))
      (assert signalled)
  ...
  (it "uses the default channel buffer size"
    (let ((stream
            (open-ollama-stream
             ...
             :post "/chat")))
      (multiple-value-bind (channel completion) (stream-channel stream)
        (multiple-value-bind (event present-p) (cl-concurrent-kit:recv channel)
          (assert present-p)
          (assert (= 1 (gethash "n" event))))
        (cl-concurrent-kit:await completion)
        (multiple-value-bind (eof present-p) (cl-concurrent-kit:recv channel)
          (declare (ignore eof))
          (assert (not present-p))))))
            ...
            (open-ollama-stream
             (queued-client
              (response-with-text
               (format nil "{\"n\":1}~%{\"n\":2}~%")))
             :post "/chat")))
      (multiple-value-bind (channel completion) (stream-channel stream :buffer-size 2)
        (multiple-value-bind (first first-p) (cl-concurrent-kit:recv channel)
          (assert first-p)
          (assert (= 1 (gethash "n" first))))
        (multiple-value-bind (second second-p) (cl-concurrent-kit:recv channel)
          (assert second-p)
