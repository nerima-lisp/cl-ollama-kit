...
(describe "Codec and data edge contracts"
          ...
          (it "covers raw body sizing and response stream representations"
              ...
              (let* ((input (make-string-input-stream "stream"))
                ...
                (assert
                 (eq input (ollama-kit::%response-input-stream response 32)))
                (assert
                 (equal "stream"
                      ...
                      (queued-client
                       (make-http-response :status
                                           ...
                                           (lambda ()
                                             (incf closed))))))
                (let ((caught nil))
                  (handler-case (request-json client :get "/version")
                    (ollama-api-error (condition)
                      (setf caught t)
                      (assert
                       (equal "nested failure" (ollama-error-message condition)))))
                  (assert caught))
                (assert (= 1 closed))))
